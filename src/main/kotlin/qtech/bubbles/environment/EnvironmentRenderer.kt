...
import qtech.bubbles.common.renderer.IRenderer
        ...
        for (entity in entities) {
            if (entity is Entity) {
                try {
                    ...
                    report.addCategory(CrashCategory("Entity being rendered").also {
                        it.add("Environment") { environment }
                        it.add("Entity") { entity::class.qualifiedName }
                        it.add("ID") { entity.entityId }
                        it.add("UUID") { entity.uniqueId }
                        it.add("MfD") { entity.isMarkedForDeletion }
                        it.add("Data") { entity.getState().toString() }
                    })
