...
import qtech.bubbles.mods.Scanner.ScanResult
...
import qtech.bubbles.event.BBAddonSetupEvent
...
open class ModLoader(private val loadScreen: LoadScreen) {
    private val addonFiles: MutableList<File> = ArrayList()
    val addons = ArrayList<Any>()
    val modClassLoader: ModClassLoader
    private val scanResults = HashMap<File, ScanResult>()
    fun constructMods() {
        if (ModManager.instance.namespaces.isEmpty()) {
            return
        }

        logger.info("Auto subscribing event bus subscribers")
        for (file in addonFiles) {
            val scanResult: ScanResult = scanResults[file]!!
            val classes: List<KClass<*>> = scanResult.classes[Modification.EventBusSubscriber::class]
        ...
        logger.info("Constructing mods: ${StringUtils.join(ModManager.instance.namespaces, ", ")}")
        for (obj in ModManager.instance.objects) {
            logger.info("Constructing mod: ${obj.modId}")
            loadScreen.logInfo(obj.modId)
            try {
                ...
                val constructor = addonClass.getConstructor(Logger::class.java, String::class.java, ModObject::class.java)
                constructor.isAccessible = true
                val addon = constructor.newInstance(logger, obj.annotation.namespace, obj)

                // Register java addon.
                modManager.register(obj.modId, addon!!)
...
//                LanguageMap.inject(inputStream);
                ...
                val addonCategory = CrashCategory("Mod was being constructed")
                addonCategory.add("Mod ID") { obj.modId}
    ...
    fun addonSetup() {
        ...
        for (locale in locales) {
            ...
            for (`object` in ModManager.instance.objects) {
...
//            Class<?> c = addonClassLoader.loadClass(className);
            ...
            val scanner = Scanner(source, modClassLoader)
            ...
            val classes: List<KClass<*>> = scanResult.getClasses(Modification::class)
            println("Mod classes: $classes")

            for (clazz in classes) {
                var addon: Modification? = null
                for (annotation in clazz.annotations) {
                    if (annotation is Modification) {
                        addon = annotation
                        break
                    }
                }
                ...
                if (addon != null) {
                    ...
                    require(flag) { "Mod has no constructor with 3 parameters. (" + clazz.qualifiedName + ")" }
                    ResourceLocation.testNamespace(addon.namespace)
                    internalAddons++

                    @Suppress("UNCHECKED_CAST")
                    val modContainer: ModContainer = object : ModContainer() {
                        override val info = ModInfo(this)
                        ...
                        override val clazz: Class<*> = clazz.java
                        override val kotlinClass: KClass<*> = clazz
                        override val obj: ModObject<out ModInstance> = ModObject(addon.namespace, this, addon, clazz.java as Class<out ModInstance>)
                        override val scanResult: ScanResult = scanResult
                        override val instance: ModInstance
                            get() = obj.addon
                    }

                    ModManager.instance.register(addon.namespace, modContainer)
            ...
            val crashReport = CrashReport(t)
            val addonCategory = CrashCategory("Mod Loading")
    ...
    init {
        ...
        modClassLoader = ModClassLoader(BubbleBlaster.mainClassLoader)
        if (!addonsDir.exists()) {
            existsBefore = false
            val flag = addonsDir.mkdirs()
            check(flag) { "Mods directory wasn't created. (" + addonsDir.path + ")" }
        } else if (addonsDir.isFile) {
            existsBefore = false
            val flag1 = addonsDir.delete()
            val flag2 = addonsDir.mkdir()
            check(flag1) { "Mods directory wasn't deleted. (" + addonsDir.path + ")" }
            check(flag2) { "Mods directory wasn't created. (" + addonsDir.path + ")" }
        }
