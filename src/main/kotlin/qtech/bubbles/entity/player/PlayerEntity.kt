...
package qtech.bubbles.entity.player

import com.google.common.annotations.Beta
import qtech.bubbles.BubbleBlaster
import qtech.bubbles.common.ability.AbilityContainer
...
import qtech.bubbles.init.AmmoTypes
import qtech.bubbles.init.Entities
import qtech.bubbles.item.inventory.PlayerInventory
...
class PlayerEntity(gameMode: AbstractGameMode?) : DamageableEntity(Entities.PLAYER.get(), gameMode!!) {
    ...
    var scoreMultiplier: Float
        ...
        set(value) {
            attributeMap.set(Attribute.SCORE_MULTIPLIER, value)
        }

    var baseScoreMultiplier: Float
        get() = bases[Attribute.SCORE_MULTIPLIER]
        private set(value) {
            bases.set(Attribute.SCORE_MULTIPLIER, value)
        }

    var luck: Float
        get() = attributeMap[Attribute.LUCK]
        set(value) {
            attributeMap.set(Attribute.LUCK, value)
        }

    var baseLuck: Float
        get() = bases[Attribute.LUCK]
        private set(value) {
            bases.set(Attribute.LUCK, value)
        }
    ...
    override fun damage(value: Double, source: EntityDamageSource?) {
        ...
        if (value > 0.0) {
            ...
            if (!BubbleBlaster.instance.isFocused) {
                BubbleBlaster.instance.requestWindowUserAttention()
            }
        }
    }
    ...
    init {
        ...
        // Set attributes.
        attributeMap.set(Attribute.DEFENSE, 1f)
        attributeMap.set(Attribute.ATTACK, 10f)
        attributeMap.set(Attribute.MAX_DAMAGE, 100f)
        attributeMap.set(Attribute.SPEED, 16f)
