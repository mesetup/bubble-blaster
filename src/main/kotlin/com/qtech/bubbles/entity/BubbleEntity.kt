...
//        }
    ...
    override val shape: Ellipse2D
        get() {
            val colors: Array<out Color> = bubbleType.colors ?: throw NullPointerException("Property colors of bubble ${bubbleType.registryName} is null")
...
//                double scoreValue = ((visibleValue * (nonVisibleValue + 1)) * scoreMultiplier * game.getLoadedGame().getGameType().getScoreMultiplier() * player.getScoreMultiplier()) * value / 32;
    ...
    companion object {
        private const val RADIUS_MOD = 6
    ...
    init {
        ...
        if (colors != null) {
            ...
            throw NullPointerException("Property colors of bubble ${properties.type.registryName} is null.")
        }

        // Set attributes.
        attributeMap.set(Attribute.MAX_DAMAGE, properties.damageValue)
        attributeMap.set(Attribute.ATTACK, properties.attack)
        attributeMap.set(Attribute.DEFENSE, properties.defense)
        attributeMap.set(Attribute.SCORE_MULTIPLIER, properties.scoreMultiplier)
        attributeMap.set(Attribute.SPEED, properties.speed)

        // Set fields.
        speed = properties.speed
        baseSpeed = properties.speed
        baseRadius = properties.radius
        damageValue = properties.damageValue
