...
import com.qsoftware.bubbles.common.crash.CrashCategory;
import com.qsoftware.bubbles.common.crash.CrashReport;
import com.qsoftware.bubbles.common.text.translation.LanguageMap;
import com.qsoftware.bubbles.event.old.ADLInitializationEvent;
import com.qsoftware.bubbles.event.old.ADLPostInitializationEvent;
import com.qsoftware.bubbles.event.old.ADLPreInitializationEvent;
import com.qsoftware.bubbles.registry.AddonRegistry;
import com.qsoftware.bubbles.registry.LocaleRegistry;
...
//    private final List<AddonContainer> containers = new ArrayList<>();
    ...
    public AddonLoader(LoadScene loadScene) {
        ...
        try {
            ...
            boolean flag = loadJar(file.getPath(), loadScene);
            if (!flag) {
                LOGGER.info("No flag.");
                loadScene.logInfo(String.format("Found non-addon file: %s", file.getName()));
    ...
    public void constructAddons() {
        if (AddonRegistry.instance().keys().isEmpty()) {
            return;
        }

        AddonManager addonManager = AddonManager.getInstance();

        LOGGER.info(String.format("Constructing Mods: %s", StringUtils.join(AddonRegistry.instance().keys(), ", ")));

        AddonObject<?> addonObject;
        for (AddonObject<?> object : AddonRegistry.instance().values()) {
            loadScene.logInfo(object.getNamespace());
            try {
...
//                LanguageMap.inject(inputStream);
    ...
    public void preInitialize() {
        QBubbles.getEventBus().post(new ADLPreInitializationEvent(this));
    }

    public void initialize() {
        QBubbles.getEventBus().post(new ADLInitializationEvent(this));
    }

    public void postInitialize() {
        QBubbles.getEventBus().post(new ADLPostInitializationEvent(this));

        String[] locales = Locale.getISOLanguages();
        for (String locStr : locales) {
            Locale loc = new Locale(locStr);
            LanguageMap langMap = new LanguageMap(loc);

            String resourcePath = "/assets/qbubbles/lang/" + loc.toString().toLowerCase() + ".lang";
...
//                LOGGER.warn(String.format("Cannot find language file: %s", resourcePath));
            ...
            for (AddonObject<?> object : AddonRegistry.instance().values()) {
                ...
                resourcePath = "/assets/" + addonId + "/lang/" + loc.toString().toLowerCase() + ".lang";
...
//                    LOGGER.warn(String.format("Cannot find language file: %s", resourcePath));
            ...
            LocaleRegistry.getRegistry().register(loc, langMap);
        }
...
//                Game.getLogger().warn(String.format("Cannot find language file: %s", resourcePath));
    ...
    public boolean loadJar(String pathToJar, @Nullable LoadScene loadScene) {
        try {
...
//            Class<?> c = addonClassLoader.loadClass(className);
            ...
            if (loadScene != null) {
                loadScene.logInfo(String.format("Scanning jar file: ", file.getPath()));
            }
            Scanner.ScanResult scanResult = scanner.scanJar(loadScene);
            ...
            for (Class<?> clazz : classes) {
                ...
                if (addon != null) {
                    ...
                    AddonManager.registerContainer(addonContainer);
                    AddonRegistry.instance().register(addon.addonId(), addonContainer.getAddonObject());
            ...
            if (sb.length() > 0) {
                ...
                throw new IllegalStateException("Missing addon classes with ids: " + sb);
            }
